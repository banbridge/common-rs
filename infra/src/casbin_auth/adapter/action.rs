use casbin::{Error as CasbinError, Filter, Result, error::AdapterError};
use sea_orm::{
...
impl<'a> Rule<'a> {
    pub fn from_str(value: &'a [&'a str]) -> Self {
    ...
    pub fn from_string(value: &'a [String]) -> Self {
        #[allow(clippy::get_first)]
        Rule {
            v0: value.get(0).map_or("", |x| x),
            v1: value.get(1).map_or("", |x| x),
...
impl<'a> RuleWithType<'a> {
    pub fn from_rule(ptype: &'a str, rule: Rule<'a>) -> Self {
        RuleWithType {
            ptype,
            v0: rule.v0,
            v1: rule.v1,
...
pub async fn remove_policy<'conn, 'rule, C: ConnectionTrait>(
    conn: &'conn C,
...
pub async fn remove_policies<'conn, 'rule, C: ConnectionTrait>(
    conn: &'conn C,
...
pub async fn remove_filtered_policy<'conn, 'rule, C: ConnectionTrait>(
    conn: &'conn C,
    ptype: &'rule str,
...
pub async fn load_policy<C: ConnectionTrait>(conn: &C) -> Result<Vec<entity::Model>> {
    entity::Entity::find()
        .all(conn)
        .await
        .map_err(|err| CasbinError::from(AdapterError(Box::new(err))))
...
pub async fn load_filtered_policy<'conn, 'filter, C: ConnectionTrait>(
    conn: &'conn C,
...
pub async fn save_policies<'conn, 'rule, C: ConnectionTrait>(
    conn: &'conn C,
    rules: Vec<RuleWithType<'rule>>,
) -> Result<()> {
...
pub async fn add_policy<'conn, 'rule, C: ConnectionTrait>(
    conn: &'conn C,
    rule: RuleWithType<'rule>,
) -> Result<bool> {
...
pub async fn add_policies<'conn, 'rule, C: ConnectionTrait>(
    conn: &'conn C,
    rules: Vec<RuleWithType<'rule>>,
) -> Result<bool> {
    for rule in rules {
...
pub async fn clear_policy<C: ConnectionTrait>(conn: &C) -> Result<()> {
